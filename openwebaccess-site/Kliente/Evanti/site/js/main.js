...
(function () {
  ...
  function handleScroll() {
    ...
    if (topBar) {
      topBar.style.transform = scrollY > 40 ? 'translateY(-100%)' : 'translateY(0)';
      topBar.style.transition = 'transform .3s ease';
    }
    // Navbar offset
    if (navbar) {
      navbar.style.top = scrollY > 40 ? '0' : topBar ? topBar.offsetHeight + 'px' : '0';
    }
  }
  // Initial position
  if (navbar && topBar) {
  ...
  if (prevBtn) prevBtn.addEventListener('click', function () { goToTestimonial(currentTestimonial - 1); });
  if (nextBtn) nextBtn.addEventListener('click', function () { goToTestimonial(currentTestimonial + 1); });
  ...
  if (testimonialSection) {
    testimonialSection.addEventListener('mouseenter', function () { clearInterval(testimonialTimer); });
